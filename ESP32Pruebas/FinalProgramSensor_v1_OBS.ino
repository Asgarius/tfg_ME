...
void inicioRGB_AL(int j, int ROT = 1){    //Inicio del led en modo rotación
  do{
    for (int i = 0; i < LEDS_COUNT; i++){
      strip.setLedColorData(i, m_color[j][1], m_color[j][1], m_color[j][2]);// Set color data.
      strip.show();
...
void controller(unsigned long value){
  switch (value) {
    ...
    //case 1: MODE NORMAL --> blue = 2, white = Estab
    if(!checkSensors()){
      inicioRGB(BlueColor);
    }
    break;
    ...
    }while(0); // Compara el pin que recibe del arduino
    break;
